...
for log_file in log_files:
    ...
    with open(log_file, "r") as file:
        ...
        use_liger = filename.split("_")[3]
        print(use_liger)
        provider = "liger" if use_liger == "True" else "huggingface"
        ...
        elif provider not in all_data[model]:
        ...
        elif batch_size not in all_data[model][provider]:
        ...
        if lines:
            ...
            try:
                ...
                total_peak_memory = data.get("total_peak_memory_allocated_MB", "OOM")
                avg_tokens_per_second = data.get("avg_tokens_per_second", "OOM")
    ...
    for provider, batch_sizes in providers.items():
        for batch_size, data in batch_sizes.items():
            ...
            if len(total_peak_memory_allocated_MB_values) > 1 and len(avg_tokens_per_second_values) > 1:
                if "OOM" in total_peak_memory_allocated_MB_values:
                    total_peak_memory_allocated_MB_mean, total_peak_memory_allocated_MB_std = "OOM", "OOM"
                    avg_tokens_per_second_mean, avg_tokens_per_second_std = "OOM", "OOM"
                else:
