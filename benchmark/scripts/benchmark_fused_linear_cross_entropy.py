...
def bench_memory_fused_linear_cross_entropy(
    ...
    mem_mean, mem_std = _test_memory(full, _iter=10)
...
def bench_speed_fused_linear_cross_entropy(
    ...
    if mode == "forward":
        time_mean = triton.testing.do_bench(fwd, rep=100, return_mode="mean")
    elif mode == "backward":
        y = fwd()
        ...
        time_mean = triton.testing.do_bench(full, rep=100, return_mode="mean")
    return SingleBenchmarkRunOutput(
        y_mean=time_mean,
    )
